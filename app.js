...
function highlightActiveNav() {
  ...
  const currentPage = page.split("?")[0].split("#")[0]; // Remove query params e anchors
  qsa(".nav-links a, .mobile-menu a").forEach((a) => {
    const href = a.getAttribute("href");
    if (!href || href.startsWith("http") || href.startsWith("#")) return; // Ignora URLs externas e âncoras
    const hrefPage = href.split("?")[0].split("#")[0]; // Remove query params e anchors
    if (hrefPage === currentPage || (currentPage === "" && hrefPage === "index.html")) a.classList.add("active");
  });
}
